        ...
        for i, (sent, label) in enumerate(train_data_loader):
            if i < args.train_steps_per_epoch:
                ...
                if (args.run_validation
                    and i % args.validation_steps_per_epoch == 0):
...
if __name__ == '__main__':
    ...
    while (retries < 5 and not successful_download):
        try:
            ...
            print('Retry #{}.  Sleeping for {} seconds'.format(retries,
                                                               random_sleep))
    ...
    train_data_loader, df_train = create_data_loader(
        args.train_data, args.train_batch_size)
    val_data_loader, df_val = create_data_loader(
        args.validation_data, args.validation_batch_size)
    ...
    # You will need three files for it: inference.py, requirements.txt,
    # config.json
